...
for f in *COG20Category.txt; do cut -f4 $f| sort | uniq -c > ${f}COG20CategoryCounts.txt; done
for f in *COG20CategoryCounts.txt; do sed 's/^ *//g' $f > ${f}COG20CountsReformatted.txt;done

###for all files (f) with the suffix KOfamCountsReformatted.txt; run the sed command to place a semicolon between gene name and counts for that gene and export (>) to a new file ${f}KOfamCountsReformattedTwo.txt###
for f in *COG20CountsReformatted.txt; do sed 's/ /;/' $f > ${f}COGCountsReformattedTwo.txt; done

###for all files (f) with the suffix KOfamCountsReformattedTwo.txt, do awk to split columns one and two into two new cells at the semi colon by gene name ($1 column1) and gene counts ($2 column2) to a new file called ${f}KOfamFinalCounts.tsv###
for f in *COGCountsReformattedTwo.txt; do awk -F ";" '{n=split($1,a,";");for (i=1;i<=n;i++) print $2"\t"a[i]}' $f > ${f}COGFinalCounts.tsv; done

for f in *Cazyme.txt; do awk -F "\t" '{ if(($5 != 0) && ($5 <= .0001)) { print } }' $f >${f}_CazymeEvalue_filtered.txt;done

###for all files (f) with the *CazymeEvalue_filtered.txt suffix, cut all genes with e-values <=0.0001 from the previous step, sort them alphabetically (sort), retain only uniq values (uniq -c) while counting the number of appearances of each gene and export (>) to a new file named ${f}CazymeCounts.txt###
for f in *CazymeEvalue_filtered.txt; do cut -f4 $f| sort | uniq -c > ${f}CazymeCounts.txt; done

###for all files (f) with the suffix CazymeCounts.txt; run the sed command to remove white space introduced by awk and export (>) to a new file ${f}CazymeCountsReformatted.txt###
for f in *CazymeCounts.txt; do sed 's/^ *//g' $f > ${f}CazymeCountsReformatted.txt;done

###for all files (f) with the suffix CazymeCountsReformatted.txt; run the sed command to place a semicolon between gene name and counts for that gene and export (>) to a new file ${f}CazymeCountsReformattedTwo.txt###
for f in *CazymeCountsReformatted.txt; do sed 's/ /;/' $f > ${f}CazymeCountsReformattedTwo.txt; done

###for all files (f) with the suffix CazymeCountsReformattedTwo.txt, do awk to split columns one and two into two new cells at the semi colon by gene name ($1 column1) and gene counts ($2 column2) to a new file called ${f}CazymeFinalCounts.tsv###
for f in *CazymeCountsReformattedTwo.txt; do awk -F ";" '{n=split($1,a,";");for (i=1;i<=n;i++) print $2"\t"a[i]}' $f > ${f}CazymeFinalCounts.tsv; done

for gene_file in `ls *CazymeFinalCounts.tsv`; do
  cut -f1 ${gene_file} >> all_Cazyme.txt
done

sort -u all_Cazyme.txt > uniq_Cazymes.txt


set -e

uniq_genes_file="uniq_Cazymes.txt"

output_file="combined-outputCazymes.tsv"
...
for curr_file in $(ls *CazymeFinalCounts.tsv); do
...
find . -name "*CazymeCounts.txt" -type f -delete
find . -name "*CazymeEvalue_filtered.txt" -type f -delete
find . -name "*CazymeFinalCounts.tsv" -type f -delete

for gene_file in `ls *COGFinalCounts.tsv`; do
  cut -f1 ${gene_file} >> allcog_genes.txt
done
...
sort -u allcog_genes.txt > uniq_cog.txt
...
for curr_file in $(ls *COGFinalCounts.tsv); do
